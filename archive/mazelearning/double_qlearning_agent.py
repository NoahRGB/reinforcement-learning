...
class DoubleQLearningAgent(Agent):
    ...
    def get_best_action(self, state, qtable):
        y, x = state
    ...
    def iteration_step(self):
        ...
        if random.random() < 0.5:
            max_q = self.qtable2[new_state_y, new_state_x, self.get_best_action(new_state, self.qtable1)]
            self.qtable1[current_state_y, current_state_x, action] += self.step_size * (reward + self.discount_factor * max_q - self.qtable1[current_state_y, current_state_x, action])
        else:
            max_q = self.qtable1[new_state_y, new_state_x, self.get_best_action(new_state, self.qtable2)]
            self.qtable2[current_state_y, current_state_x, action] += self.step_size * (reward + self.discount_factor * max_q - self.qtable2[current_state_y, current_state_x, action])
