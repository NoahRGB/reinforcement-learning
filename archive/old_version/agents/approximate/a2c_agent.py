...
class CombinedNN(nn.Module):
    def __init__(self, state_space_dim, action_space_dim, cont):
        ...
        if self.cont:
            ...
            self.log_sigma_head = nn.Parameter(torch.ones(*action_space_dim) * -1.5)
        else:
    ...
    def forward(self, inp):
        ...
        if self.cont:
            ...
            log_sigma = self.log_sigma_head.clamp(-20, 2)
...
class A2CAgent(Agent):
    def __init__(self, device, logger, lr, gamma, lam, rmsprop_eps, conv, cont,
    ...
    def run_policy(self, s, t):
        ...
        if self.cont:
            ...
            action = dist.sample().cpu().numpy() # (num_envs, action_space_dim,)
    ...
    def initialise(self, state_space, action_space, start_state, num_envs):
        ...
        self.optimiser = optim.RMSprop(self.combined_nn.parameters(), lr=self.lr, eps=self.rmsprop_eps)
    ...
    def make_a2c_update(self):
        ...
        _, next_state_values = self.combined_nn(sprime) # (tmax, num_envs, action_space_dim), (tmax, num_envs, 1)

        state_values = state_values.squeeze(-1) # (tmax, num_envs)
        next_state_values = next_state_values.squeeze(-1).detach() # (tmax, num_envs)
