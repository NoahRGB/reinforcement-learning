...
class QSigmaOffPolicyNstepAgent(Agent):
    ...
    def iteration_step(self):
        ...
        if not self.done:
            ...
            self.sigmas[self.time_step] = 0.5
            if self.done: self.termination_time = self.time_step
            ...
            self.isrs[self.time_step] = target_probability / behaviour_probability
        ...
        if time_to_update >= 0:
            self.last_time_updated = time_to_update

            state_to_update_y, state_to_update_x = self.states[time_to_update]
            action_to_update = self.actions[time_to_update]

            target = 0
            for t in range(min(self.time_step, self.termination_time), time_to_update, -1):
                if t == self.termination_time:
                    ...
                    chosen_state_y, chosen_state_x = self.states[t]
                    chosen_action = self.actions[t]
                    ...
                    target = (
                            ...
                            self.discount_factor * (self.sigmas[t]*self.isrs[t] + (1-self.sigmas[t])*chosen_action_prob) * (target - self.qtable[chosen_state_y, chosen_state_x, chosen_action])
                    )
    ...
    def learn(self, iterations, quiet=False):
        for episode in range(iterations):
            self.reset_iteration()
            while self.last_time_updated < (self.termination_time):
                self.iteration_step()
            self.trajectory_length_history.append(self.target_policy_time_step)
