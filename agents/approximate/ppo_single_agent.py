...
class PPOSingleAgent(Agent):
    ...
    def make_ppo_update(self):
        ...
        s = torch.as_tensor(np.array(self.transitions["s"]), dtype=torch.float32).to(self.device) # (tmax, state_space_dim)
        a = torch.as_tensor(np.array(self.transitions["a"]), dtype=torch.int64).to(self.device) # (tmax,)
        r = torch.as_tensor(np.array(self.transitions["r"]), dtype=torch.float32).to(self.device) # (tmax,)
        sprime = torch.as_tensor(np.array(self.transitions["sprime"]), dtype=torch.float32).to(self.device) # (tmax, state_space_dim)
        done = torch.as_tensor(np.array(self.transitions["done"]), dtype=torch.float32).to(self.device) # (tmax,)
        ...
        surrogate_obj = ratios * advantages.detach() # (tmax,)
        clipped_surrogate_obj = clipped_ratios * advantages.detach() # (tmax,)
    ...
    def update(self, s, sprime, a, r, done):
        ...
        with torch.no_grad():
            log_probs, _ = self.combined_nn(self.process_state(s[0]).unsqueeze(0)) # (1, action_space_dim), (1, 1)
