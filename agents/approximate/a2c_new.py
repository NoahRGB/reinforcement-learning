...
class A2Cnew(Agent):
    def __init__(self, device, writer, actor_lr, critic_lr, gamma, cont,
    ...
    def initialise(self, state_space, action_space, start_state, num_envs):
        ...
        self.actor = Actor(self.state_space_size, self.action_space_size, self.cont).to(self.device)
        self.critic = Critic(self.state_space_size, self.action_space_size).to(self.device)
    ...
    def make_a2c_update(self):
        ...
        prev_term = last_state_value
        for t in reversed(range(len(self.transitions))):
            this_return = r[t] + self.gamma * prev_term * masks[t]
            returns[t] = this_return
            prev_term = this_return
