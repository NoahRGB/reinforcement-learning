...
class A2CSingleContinuousAgent(Agent):
    ...
    def run_policy(self, s, t):
        if self.cont:
            ...
            with torch.no_grad():
                (mu, sigma), _ = self.combined_nn(self.process_state(s)) # (1, action_space_dim), (1, action_space_dim)
                mu = mu.cpu().squeeze(0).numpy() # (action_space_dim,)
                sigma = sigma.cpu().squeeze(0).numpy() # (action_space_dim,)
                return [np.random.normal(mu, sigma)] # (action_space_dim,)
        else:
    ...
    def make_a2c_update(self):
        ...
        a = torch.tensor(np.array(self.transitions["a"]), dtype=torch.int64 if not self.cont else torch.float32).to(self.device) # (tmax,)
        ...
        if not self.cont:
            ...
            (mu, sigma), state_values = self.combined_nn(s) # (tmax, action_space_dim), (tmax, 1)
            state_values = state_values.squeeze(1) # (tmax,)
            dist = torch.distributions.Normal(mu, sigma)
            chosen_log_probs = dist.log_prob(a).sum(dim=-1) # (tmax,)
        ...
        if not self.cont:
            entropy_sum = (-torch.exp(chosen_log_probs) * chosen_log_probs).sum()
