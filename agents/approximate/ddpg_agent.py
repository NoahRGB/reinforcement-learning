...
class DDPGAgent(Agent):
    ...
    def update_qfuncs(self, all_s, all_a, all_r, all_sprime, all_done):
        ...
        with torch.no_grad():
            # target_policy_mus, target_policy_sigmas = self.target_actor(all_sprime) # (minibatch_size, action_space_dim,)
            # target_policy_dists = torch.distributions.Normal(target_policy_mus, target_policy_sigmas)
            # target_policy_actions = torch.tanh(target_policy_dists.rsample()) # (minibatch_size, action_space_dim,)
    ...
    def update_actor(self, all_s):
        ...
        # policy_mus, policy_sigmas = self.actor(all_s) # (minibatch_size, action_space_dim,)
        # policy_dists = torch.distributions.Normal(policy_mus, policy_sigmas)
        # policy_actions = torch.tanh(policy_dists.rsample()) # (minibatch_size, action_space_dim,)
