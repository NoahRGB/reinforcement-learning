...
class CombinedNN(nn.Module):
    ...
    def forward(self, input):
        return self.policy_nn(input), self.state_value_nn(input)
...
class A2CAgent(Agent):
    ...
    def run_policy(self, s, t):
        with torch.no_grad():
            probs, _ = self.combined_nn(self.process_state(s))
    ...
    def make_update(self):
        ...
        if not terminal:
            R = self.combined_nn(self.process_state(final_s))[1].squeeze().detach()

        for i in reversed(range(len(self.transitions))):
            (s, sprime, a, r, done)  = self.transitions[i]
            R = r + self.gamma * R

            policy_prediction, state_value_prediction = self.combined_nn(self.process_state(s))
